      ...
      var i = 0,
        ...
        isBackspacing = false,
        isParagraph = false;
      var textArray = ["Hệ thống Lì Xì MoMo|Uy tín, giao dịch tự động 24/7 !"];
      var speedForward = 0,
        speedWait = 30000,
      ...
      function copyStringToClipboard(str) {
        ...
        document.body.removeChild(el);
         swal("Thông báo", "Đã sao chép số điện thoại này. Chúc bạn may mắn.!!",
                                            "success");
      }
...
check_sdt();
    ...
    function choilanhan(){if(!$('#PhoneChoi')['val']())
    swal("Lỗi!", "Bạn chưa nhập số điện thoại!!", "error");
else {
    $(document).ajaxStart(function () {
            $("#submit1").attr("disabled", true);
        });    
    nap()}}
function nap(){$('#submit1')['html']('<div class="spinner-border text-warning" role="status"></div>Đang xử lý');
$['post']('/kt1',{
    PhoneChoi:$('#PhoneChoi')['val']()},
    function(_0xb982x3){swal(_0xb982x3['ketqua']);
    $('#submit1')['html']('SĐT khác')},'json')    
    $(document).ajaxComplete(function() {
            $("#submit1").attr("disabled", false);
        });    
}

function choilanhan2(){if(!$('#PhoneChoia')['val']())
 swal("Lỗi!", "Bạn cần cung cấp mã giao dịch hoặc sđt để kiểm tra !", "error");
    ...
    nap2()}}
function nap2(){$('#submit2')['html']('<div class="spinner-border text-warning" role="status"></div>Đang xử lý');
$['post']('/kt2',{
    PhoneChoia:$('#PhoneChoia')['val']()},
    function(_0xb982x3){swal(_0xb982x3['ketqua']);
    $('#submit2')['html']('Mã GD hoặc SĐT khác')},'json')    
    $(document).ajaxComplete(function() {
            $("#submit2").attr("disabled", false);
        });    
}
function choilanhan3(){if(!$('#PhoneChoia')['val']()){swal('Bạn cần cung cấp mã giao dịch để web xác định trả thưởng !');}
else if(!$('#phonenhan')['val']()){swal('Bạn cần cung cấp số điện thoại để nhận thưởng !');}
else {
    $(document).ajaxStart(function () {
            $("#submit3").attr("disabled", true);
        });    
    nap3()}}
function nap3(){$('#submit3')['html']('<div class="spinner-border text-warning" role="status"></div>Đang xử lý');
$['post']('/kt3',{
    PhoneChoia:$('#PhoneChoia')['val'](),
    phonenhan:$('#phonenhan')['val']()
},
    function(_0xb982x3){swal(_0xb982x3['ketqua']);
    $('#submit3')['html']('Số điện thoại khác')},'json')    
    $(document).ajaxComplete(function() {
